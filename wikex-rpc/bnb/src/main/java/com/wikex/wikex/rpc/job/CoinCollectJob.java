...
//    }
    ...
    /**
     * Recharge miner fees for user addresses (executed every two hours)
     */
    @Scheduled(cron = "0 */5 * * * *")
    public void rechargeMinerFee() {
        try {
            logger.info("Recharge miner fees for user addresses (executed every 1 hour)");
            AccountReplay accountReplay = new AccountReplay(accountService, 100);
            ...
            accountReplay.run(account -> {
                try {
                    ...
                    for (Contract coin : contracts) {
                        logger.info("Recharge miner fees for user address: " + JSON.toJSONString(coin));
                        BigDecimal tokenBalance = ethService.getTokenBalance(account.getAddress(), coin);
                        logger.info("Get balance: " + tokenBalance.toPlainString());
                    ...
                    // Give gas fee to addresses that meet the conditions:
                    ...
                    if (needTransferFee) {
                        ...
                        // Calculate the miner fee amount to transfer this time
