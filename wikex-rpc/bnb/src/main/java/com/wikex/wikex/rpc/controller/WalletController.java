...
import com.wikex.wikex.rpc.component.BNBWatcher;
...
public class WalletController {
    ...
    public MessageResult getNewAddress(@PathVariable String account) {
        try {
            String address;
            Account acct = accountService.findByName("BNB", account);
            if (acct != null) {
    ...
    public MessageResult transfer(String password, String address, String coinName, BigDecimal amount, BigDecimal fee) {
        ...
        if (coinName == null || coinName.equals(coin.getUnit()) || coinName.equals(coin.getName())) {
            try {
            ...
            try {
                ...
                for (Account account : accountList) {
                    if (service.getBalance(account.getAddress()).compareTo(fee) < 0) {
                        logger.info("Address {} has insufficient fee, minimum is {}", account.getAddress(), fee);
                        continue;
                    }
                    ...
                    if (availAmt.compareTo(contract.getMinCollectAmount()) < 0) {
                        logger.info("Address {} has insufficient balance, minimum is {}", account.getAddress(), contract.getMinCollectAmount());
                        continue;
                    }
                ...
                logger.info("Return result : " + mr.toString());
                return mr;
            } catch (Exception e) {
                e.printStackTrace();
                return MessageResult.error(500, "error:" + e.getMessage());
