...
public class TronUtils {
	...
	public static String toHexAddress(String address) {
		if (StringUtils.isEmpty(address)) {
			throw new IllegalArgumentException("Input address cannot be empty");
		}
		if (!address.startsWith("T")) {
			throw new IllegalArgumentException("Invalid input address:" + address);
		}
	 ...
	 *
	 * @return address information map
	 */
	...
	public static String signAndBroadcast(String tronUrl, String privateKey, JSONObject transaction) throws Throwable {
		...
		String signTransaction = Hex.toHexString(bytes);
		JSONObject jsonObjectGB = new JSONObject();
		jsonObjectGB.put("transaction", signTransaction);
		String url = tronUrl + "/wallet/broadcasthex";
		String transactionComplete1 = HttpClientUtils.postJson(url, jsonObjectGB.toString());
		JSONObject transactionComplete = JSONObject.parseObject(transactionComplete1);
		if (transactionComplete.getBoolean("result")) {
			return transactionComplete.getString("txid");
		} else {
			logger.error(String.format("Failed to sign transaction:%s", transactionComplete1));
			return null;
		}
	}
	 ...
	 */
	public static byte[] signTransactionByte(byte[] transaction, byte[] privateKey)
	...
	/**
	 * Convert hex address to base58 view address
	 *
	...
	/**
	 * Calculates the SHA-256 or SM3 hash of the given bytes.
	 *
	 * @param isSha256 true for SHA-256, false for SM3
	...
	/**
	 * Calculates the SHA-256 or SM3 hash of the given byte range.
	 *
	 * @param isSha256 true for SHA-256, false for SM3
	...
	/**
	 * Pack transaction from JSON string
	 *
	 * @param strTransaction transaction JSON string
	 * @return Protocol.Transaction object
	 */
			...
			try {
				...
				switch (contractType) {
					...
					case "VoteAssetContract":
						VoteAssetContractOuterClass.VoteAssetContract.Builder voteAssetContractBuilder = VoteAssetContractOuterClass.VoteAssetContract
								.newBuilder();
						JsonFormat
					...
					case "AssetIssueContract":
						AssetIssueContractOuterClass.AssetIssueContract.Builder assetIssueContractBuilder = AssetIssueContractOuterClass.AssetIssueContract
								.newBuilder();
						JsonFormat
					...
					case "ParticipateAssetIssueContract":
						AssetIssueContractOuterClass.ParticipateAssetIssueContract.Builder participateAssetIssueContractBuilder = AssetIssueContractOuterClass.ParticipateAssetIssueContract
								.newBuilder();
						JsonFormat.merge(parameter.getJSONObject("value").toString(),
					...
					case "UpdateAssetContract":
						...
						break;
					case "SmartContract":
						SmartContractOuterClass.SmartContract.Builder smartContractBuilder = SmartContractOuterClass.SmartContract
								.newBuilder();
						JsonFormat
					...
					case "TriggerSmartContract":
						...
						break;
					// todo: add other contract types if needed
					default:
				}
				if (any != null) {
