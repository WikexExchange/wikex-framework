...
import com.wikex.wikex.rpc.service.EtherscanApi;
...
public class EthWatcher extends Watcher {
    ...
    private void ethDeposits(List<Deposit> deposits, Transaction transaction, String to) {
        ...
        // If the address is sending from the address book, sync balance
    ...
    private void tokenDeposits(List<Deposit> deposits, Transaction transaction, Long blockHeight) {
        ...
        if (StringUtils.isNotEmpty(input) && input.length() >= 138) {
            ...
            if (accountService.isAddressExist(toAddress)) {
                ...
                // When eventTopic0 parameter is not empty, check event_log results to prevent fake deposits from older token versions
