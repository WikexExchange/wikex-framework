 ...
 * handler thread
 * 
 * @author william 2016-1-16 19:52:47
 */
public class JobThread extends Thread {
	...
	public void toStop(String stopReason) {
		/**
		 * Thread.interrupt only supports terminating a thread in a blocked state (wait,
		 * join, sleep).
		 * Throwing an InterruptedException exception during a blocked state does not
		 * terminate the running thread itself.
		 * Therefore, it is important to note that to completely destroy the thread
		 * here, a shared variable must be used.
		 */
		this.toStop = true;
