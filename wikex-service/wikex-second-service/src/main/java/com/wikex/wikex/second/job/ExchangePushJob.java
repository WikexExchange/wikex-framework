...
public class ExchangePushJob {
    ...
    public void pushTrade(){
        ...
        while (entryIterator.hasNext()){
            ...
            if(trades.size() > 0){
                synchronized (trades) {
                    messagingTemplate.convertAndSend("/topic/second/trade/" + symbol, trades);
            ...
            if(plates.size() > 0){
                ...
                synchronized (plates) {
                    for(TradePlate plate:plates) {
                        ...
                        messagingTemplate.convertAndSend("/topic/second/trade-plate/" + symbol, plate.toJSON(24));
                        
                        messagingTemplate.convertAndSend("/topic/second/trade-depth/" + symbol, plate.toJSON(50));
    ...
    public void pushThumb(){
        ...
        while (entryIterator.hasNext()){
            ...
            if(thumbs.size() > 0){
                synchronized (thumbs) {
                    messagingTemplate.convertAndSend("/topic/second/thumb",thumbs.get(thumbs.size() - 1));
                    thumbs.clear();
                }
            }
        }
