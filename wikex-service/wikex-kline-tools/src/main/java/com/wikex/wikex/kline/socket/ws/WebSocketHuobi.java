package com.wikex.wikex.kline.socket.ws;

import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.wikex.wikex.kline.entity.KLine;
import com.wikex.wikex.kline.service.KlineRobotMarketService;
import com.wikex.wikex.kline.util.JSONUtils;
import com.wikex.wikex.kline.util.ZipUtils;
...
public class WebSocketHuobi extends WebSocketClient {
    ...
    public void onClose(int arg0, String arg1, boolean arg2) {
        ...
        if (this.getReadyState().equals(ReadyState.NOT_YET_CONNECTED) || this.getReadyState().equals(ReadyState.CLOSED)
                || this.getReadyState().equals(ReadyState.CLOSING)) {

            Runnable sendable = new Runnable() {
                @Override
                public void run() {
                    logger.info("[WebSocketHuobi] ");
                    reconnect();
                }
            };
    ...
    public void onMessage(ByteBuffer bytes) {
        try {
            ...
            logger.info("[WebSocketHuobi] ===========================> receive message: {}", jsonObject);
            if (!"".equals(message)) {
                logger.info("[WebSocketHuobi] ===========================> PROCESS message: {}");
                if (message.indexOf("ping") > 0) {
                    String pong = jsonObject.toString();
                    send(pong.replace("ping", "pong"));
                    logger.info("[WebSocketHuobi] ===========================> send message: {}",
                    ...
                    if (type.equals("kline")) {
                        ...
                        if (null != data && !"".equals(data) && JSONUtils.isJsonArray(data)) {
                            ...
                            for (int i = 0; i < klineList.size(); i++) {
                                ...
                                BigDecimal close = (price == null || i == klineList.size() - 1)
                                        ? klineObj.getBigDecimal("close")
                                        : price;
