...
public interface ExchangeOrderMapper extends BaseMapper<ExchangeOrder> {
    ...
    @Select("select coin_symbol,min(completed_time) as completed_time,sum(traded_amount) as traded_amount from exchange_order where completed_time >= #{startTime} and completed_time < #{endTime} and direction = 1 and status = 1 group by coin_symbol")
    List<ExchangeOrder> getExchangeTurnoverCoin(@Param("startTime") long startTime, @Param("endTime") long endTime);

    @Select("select base_symbol,min(completed_time) as completed_time,sum(turnover) turnover from exchange_order where completed_time >= #{startTime} and completed_time < #{endTime} and direction = 1 and status = 1 group by base_symbol")
    List<ExchangeOrder> getExchangeTurnoverBase(@Param("startTime") long startTime, @Param("endTime") long endTime);

    @Select("select base_symbol,coin_symbol,min(completed_time) as completed_time,sum(traded_amount) as traded_amount,sum(turnover) as turnover from exchange_order where completed_time >= #{startTime} and completed_time < #{endTime} and direction = 1 and status = 1 group by base_symbol,coin_symbol")
