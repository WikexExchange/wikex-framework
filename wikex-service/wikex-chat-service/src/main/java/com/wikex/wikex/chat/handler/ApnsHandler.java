...
public class ApnsHandler {
    ...
    public void handleMessage(String uid, ChatMessageRecord message){
        ...
        if(StringUtils.isNotEmpty(uid) && uidTokenMap.containsKey(uid)) {
            ...
            try {
                ...
                e.printStackTrace();
                logger.error("Failed to send APNS: " + e.getMessage());
            }
        }
    }
    ...
    public void sendNotification(String token,ChatMessageRecord message) {
        ...
        String payloads = APNS.newPayload()
                .alertBody(message.getNameFrom() + ":" + message.getContent())
                .badge(1)
                .customField("addition", message)
                .sound("default")
                .build();

        service.push(token, payloads);
        logger.info("------------Push notification has been sent!");
...
//        apnsService.sendNotification(notification);// Asynchronous sending
//        logger.info("---------------APNS push completed");
//        boolean result = apnsService.sendNotificationSynch(notification);// Synchronous sending
//        logger.info("---------------Synchronous sending: " + result);
    }
}
