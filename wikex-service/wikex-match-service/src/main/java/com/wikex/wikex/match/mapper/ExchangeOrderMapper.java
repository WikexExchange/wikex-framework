...
public interface ExchangeOrderMapper extends BaseMapper<ExchangeOrder> {
    ...
    @Select("select coin_symbol,FROM_UNIXTIME(completed_time/1000, '%Y-%m-%d') as completed_time,sum(traded_amount) as traded_amount from exchange_order where completed_time >= #{startTime} and completed_time < #{endTime} and direction = 1 and status = 1 group by coin_symbol")
    List<ExchangeOrder> getExchangeTurnoverCoin(@Param("startTime") long startTime, @Param("endTime") long endTime);

    @Select("select base_symbol,FROM_UNIXTIME(completed_time/1000, '%Y-%m-%d') as completed_time,sum(turnover) turnover from exchange_order where completed_time >= #{startTime} and completed_time < #{endTime} and direction = 1 and status = 1 group by base_symbol")
    List<ExchangeOrder> getExchangeTurnoverBase(@Param("startTime") long startTime, @Param("endTime") long endTime);

    @Select("select base_symbol,coin_symbol,FROM_UNIXTIME(completed_time/1000, '%Y-%m-%d') as completed_time,sum(traded_amount) as traded_amount,sum(turnover) as turnover from exchange_order where completed_time >= #{startTime} and completed_time < #{endTime} and direction = 1 and status = 1 group by base_symbol,coin_symbol")
    List<ExchangeOrder> getExchangeTurnoverSymbol(@Param("startTime") long startTime, @Param("endTime") long endTime) ;
    ...
    List<ExchangeOrder> queryExchangeOrderByCancelTime(@Param("cancelTime") long cancelTime);
