...
import com.wikex.wikex.swap.netty.QuoteMessage;
...
public class NettyHandler implements MarketHandler {
    ...
    public QuoteMessage.SimpleResponse subscribeSymbolThumb(byte[] body, ChannelHandlerContext ctx) {
        ...
        response.setCode(0).setMessage("Subscribe success");
    ...
    public QuoteMessage.SimpleResponse unsubscribeSymbolThumb(byte[] body, ChannelHandlerContext ctx) {
        ...
        response.setCode(0).setMessage("Unsubscribe success");
        return response.build();
    }
    ...
    public void handleTrade(String symbol, CoinThumb thumb) {
        ...
        hawkPushService.pushMsg(NettyCacheUtils.getChannel(topicOfSymbol), NettyCommand.CONTRACT_PUSH_SYMBOL_THUMB,
                body);
    }
    ...
    public void handleTrades(String symbol, List<ContractTrade> exchangeTrades, CoinThumb thumb) {
        if (exchangeTrades.size() > 0) {
            hawkPushService.pushMsg(
                    ...
                    NettyCommand.CONTRACT_PUSH_EXCHANGE_TRADE,
                    JSONObject.toJSONString(exchangeTrades.get(exchangeTrades.size() - 1)).getBytes());
        }
    }
    ...
    public void handleKLine(String symbol, KLine kLine) {
        hawkPushService.pushMsg(
                NettyCacheUtils.getChannel(symbol),
                NettyCommand.CONTRACT_PUSH_EXCHANGE_KLINE,
    ...
    public void handlePlate(String symbol, TradePlate plate) {
        ...
        hawkPushService.pushMsg(
                NettyCacheUtils.getChannel(symbol),
                NettyCommand.CONTRACT_PUSH_EXCHANGE_PLATE,
                plate.toJSON(24).toJSONString().getBytes());
        // Push depth
        hawkPushService.pushMsg(
                NettyCacheUtils.getChannel(symbol),
                NettyCommand.CONTRACT_PUSH_EXCHANGE_DEPTH,
                plate.toJSON(50).toJSONString().getBytes());
    }
